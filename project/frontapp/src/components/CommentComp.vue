...
<script>
  ...
  export default {
    ...
    methods: {
      getComment(bid){
        axios.get(`http://localhost:3000/comment/${bid}`)
        .then(response => {
      ...
      if(id > 0){
        axios.put(`http://localhost:3000/comment/${id}`, param)
        .then(response => {
        ...
        axios.post('http://localhost:3000/comment', param)
        .then(response => {
          if(response.data.affectedRows > 0){
            alert('댓글이 등록되었습니다.');
            this.getComment(this.bid)
      ...
      editComment(id){
        axios.get(`http://localhost:3000/comment/info/${id}`)
        .then(response => {
